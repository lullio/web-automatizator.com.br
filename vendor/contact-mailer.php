...
//    eeu coloquei $mail->SMTPDebug = 3;
    ...
    $mail->addAddress($your_email, 'Felipe Lullio');     // Add a recipient
    ...
    if(isset($_POST["userPhone"])) {
        $mail->Body .= "<strong>Telefone: </strong>" . $user_Phone . "<br>";
    }
    if(isset($_POST["userSubject"])) {
        $mail->Body .= "<strong>Assunto: </strong>" . $user_Subject . "<br>";
    }
    if(isset($_POST["userCity"])) {
    ...
    if(isset($_POST["userMessage"])) {
        $mail->Body .= "<strong>Mensagem: </strong><br><br><div style='background-color: #EDEFF2;padding:30px 15px;border-radius:10px;min-height:50px;width:90%;'>" . $user_Message . "</div><br>";
    }
    $mail->Body .= '<strong>Obrigado,</strong><br>';
    ...
    if(!$mail->send())
    {
        ...
        $output = json_encode(array('type'=>'message', 'text' => 'Oi '.$user_Name .' , Obrigado pela mensagem!.'));
        die($output);
    }
}
?>